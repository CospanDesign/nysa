...
#       self.get_project_constraint_files()
    ...
    def initialize_graph(self, debug=False):
        ...
        try:
            ...
            if debug: print "Invalid Module Name: %s" % (host_interface_name)
        ...
        if debug: print "loading %d peripheral slaves" % sp_count
...
##                  print "found binding"
        ...
        if debug: print "loading %d memory slaves" % sm_count
          ...
          for host_name in self.project_tags["SLAVES"].keys():
            if "BUS" in self.project_tags["SLAVES"][host_name].keys():
              for arb_name in self.project_tags["SLAVES"][host_name]["BUS"].keys():
                ...
                if debug: print "arbitor: %s attaches to %s through bus: %s" % (host_name, slave_name, arb_name)
    ...
    def save_config_file(self, filename):
        ...
        try:
            ...
            file_out.close()
              ...
              raise WishboneModelError("Unable to write config file: %s" % filename)
        ...
        for i in range(0, p_count):
            ...
            if "arbitor_masters" in sc_slave.parameters.keys():
                ...
                if len(ams) > 0:
                    ...
                    for a in ams:
                        if debug: print "arbitor name: %s" % a
...
##           print "bind id: " + str(id(bindings))
            if debug: print "bind contents: " + str(bindings)
...
##      del bind_dict[port_name]
    ...
    def add_arbitor_by_name(self, host_name, arbitor_name, slave_name):
        ...
        if arbitor_name not in tags["arbitor_masters"]:
            raise WishboneModelError("%s is not an arbitor of %s" % (arbitor_name, host_name))
...
##       return True
    ...
    def get_connected_arbitor_name(self, host_type, host_index,
        ...
        #if len(slaves.keys()):
        #  raise WishboneModelError("host: %s is not connected to anything" % h_name)
        ...
        raise WishboneModelError("host: %s is not connected to %s" % (h_name, s_name))
